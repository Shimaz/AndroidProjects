...
import kr.tangomike.hoam.hdnavi20131201.TuioPoint;
...
@SuppressLint({ "HandlerLeak", "ViewConstructor", "ClickableViewAccessibility" })
public class TouchView extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static final float defaultNavScale = 146.0f * MAGNIFY; 
	private static final float defaultNavX = 408.0f * MAGNIFY;
	private static final float defaultNavY = 584.0f * MAGNIFY;
	private static final float minNavScale = 76.0f * MAGNIFY;
	private static final float maxNavScale = 303.0f * MAGNIFY;
	
	//background position
	private static final int bgX = 97 * MAGNIFY;
	private static final int bgY = 16 * MAGNIFY;
	
	//boundary limit 
	private static final int boundaryLeft = 97 * MAGNIFY;
	private static final int boundaryTop = 16 * MAGNIFY;
	private static final int boundaryRight = 703 * MAGNIFY;
	private static final int boundaryBottom = 1264 * MAGNIFY;
	...
	private static int navMargin = 100 * MAGNIFY;
	...
	public TouchView(Context context, String oscIP, int oscPort, boolean drawAdditionalInfo, boolean sendPeriodicUpdates) {
		...
		navPaint.setStrokeWidth(5 * MAGNIFY);
		...
		navShadowPaint.setStrokeWidth(5 * MAGNIFY);
		...
		backgroundImage = BitmapFactory.decodeResource(getResources(), R.drawable.uhd_dmag_bg);
			...
			public void handleMessage(Message msg){
        		...
        		if(tCounter <= screenSaverOnTime){
        			...
        			if(c != null)
        			{
        				...
        				c.drawRect(navX - navScale + 4 * MAGNIFY, navY - (navScale / 9*16) + 4 * MAGNIFY, navX + navScale + 4 * MAGNIFY, navY + (navScale / 9*16) + 4 * MAGNIFY, navShadowPaint);			
            			c.drawRect(navX - navScale, navY - (navScale / 9*16), navX + navScale, navY + (navScale / 9*16), navPaint);

            			// ui guide 
            			c.drawBitmap(arrowLeft, navX-(navScale + 10 * MAGNIFY + arrowLeft.getWidth()), navY - (arrowLeft.getHeight()/2), null);
            			c.drawBitmap(arrowRight, navX+(navScale + 10 * MAGNIFY), navY - (arrowRight.getHeight()/2), null);
            			c.drawBitmap(arrowUp, navX - (arrowUp.getWidth()/2), navY - ((navScale / 9*16) + 10 * MAGNIFY + arrowUp.getHeight()), null);
            			c.drawBitmap(arrowDown, navX - (arrowDown.getWidth()/2), navY + ((navScale / 9*16) + 10 * MAGNIFY), null);
...
//		MEOld = event.getAction();
	...
	public void sendTUIOdata () throws ArrayIndexOutOfBoundsException {
		...
		outputData[0] = (Float) navX / MAGNIFY;
		outputData[1] = (Float) navY / MAGNIFY;
