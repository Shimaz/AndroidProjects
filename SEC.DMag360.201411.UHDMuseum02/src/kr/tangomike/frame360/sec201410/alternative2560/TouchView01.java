...
public class TouchView01 extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static final float minNavScale = 78.5f * MAGNIFY;
	...
	private int bgX = 240 * MAGNIFY;
	private int bgY = 280 * MAGNIFY;
	...
	public TouchView01(Context context, String oscIP, int oscPort, boolean drawAdditionalInfo, boolean sendPeriodicUpdates) {
		...
		for (int i = 0; i < 72; i++){
			...
			int ir = getResources().getIdentifier("img2_" + str, "raw", PACKAGE_NAME);
			Bitmap tmpBmp;
...
//						difShow = 262;
			...
			if(navX >= (561 * MAGNIFY) - centerMargin){ // right
				navX = (561 * MAGNIFY) - centerMargin;
			}
			if(navY <= bgY + centerMargin){ // top
				navY = bgY + centerMargin;
			}
			if(navY >= (816 * MAGNIFY) - centerMargin){ // bottom
				navY = (816 * MAGNIFY) - centerMargin;
			}
