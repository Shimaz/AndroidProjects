...
public class TouchView02 extends SurfaceView implements SurfaceHolder.Callback {
	...
	private static final float maxNavScale = 578.0f;
	
	
	private static final int defaultNavScale = 198; 
	private static final int defaultNavX = 800;
	private static final int defaultNavY = 1096;
	private static final int minNavScale = 94;
	...
	// Boundary Limit
	private static final int boundaryLeft = 222;
	private static final int boundaryTop = 68;
	private static final int boundaryRight = 1378;
	private static final int boundaryBottom = 2124;
	
	private static final int centerMargin = 80;
	
	
	private int bgX = 370;// 185*2;
	private int bgY = 774;// 387*2;
	...
	public TouchView02(Context context, String oscIP, int oscPort, boolean drawAdditionalInfo, boolean sendPeriodicUpdates) {
		super(context, null);
...
//						difShow = 262;
			...
			if(navX >= 1230 - centerMargin){ // right
				navX = 1230 - centerMargin;
			}
			if(navY <= bgY + centerMargin){ // top
				navY = bgY + centerMargin;
			}
			if(navY >= 1560 - centerMargin){ // bottom
				navY = 1560 - centerMargin;
			}
			...
			// Shadow
			c.drawRect(navX - navScale + 8, navY - (navScale / 9*16) + 8, navX + navScale + 4, navY + (navScale / 9*16) + 8, navShadowPaint);			
			// Navigator
...
//		backgroundImage = BitmapFactory.decodeResource(getResources(), ir);
		...
		if (c != null) {
			...
			c.drawRect(boundaryLeft, boundaryTop, boundaryRight, boundaryBottom, bgPaint);
			c.drawBitmap(backgroundImage,bgX,bgY,null);
			c.drawRect(navX - navScale + 8, navY - (navScale / 9*16) + 8, navX + navScale + 8, navY + (navScale / 9*16) + 8, navShadowPaint);			
			c.drawRect(navX - navScale, navY - (navScale / 9*16), navX + navScale, navY + (navScale / 9*16), navPaint);
			getHolder().unlockCanvasAndPost(c);
		}
	...
	public void sendTUIOdata () throws ArrayIndexOutOfBoundsException {
		...
		outputData[3] = (Integer) screenSaver; //1; //        Number screenSaver;
...
//		this.width = width;
		...
		if (c != null) {
			...
			// ui guide 
			c.drawBitmap(arrowLeft, navX-(navScale + 20 + arrowLeft.getWidth()), navY - (arrowLeft.getHeight()/2), null);
			c.drawBitmap(arrowRight, navX+(navScale + 20), navY - (arrowRight.getHeight()/2), null);
			c.drawBitmap(arrowUp, navX - (arrowUp.getWidth()/2), navY - ((navScale / 9*16) + 20 + arrowUp.getHeight()), null);
			c.drawBitmap(arrowDown, navX - (arrowDown.getWidth()/2), navY + ((navScale / 9*16) + 20), null);
